...
public class AppUtil  {
	...
	public static boolean checkParam(String method, PageData pd){
		...
		}else if(method=="statusUtil"){//登录状态
			paramArray = Const.APP_LOGINSTATUS_PARAM_ARRAY;  
			valueArray = Const.APP_LOGINSTATUS_VALUE_ARRAY;
			
		}else if(method=="loginOut"){//注销
			paramArray = Const.APP_LOGINSTATUS_PARAM_ARRAY;  
			valueArray = Const.APP_LOGINSTATUS_VALUE_ARRAY;
			
		}else if(method=="findAU"){//忘记密码,验证用户是否存在
			paramArray = Const.APP_FORGOTPASSWORD_PARAM_ARRAY;
			valueArray = Const.APP_FORGOTPASSWORD_VALUE_ARRAY;
