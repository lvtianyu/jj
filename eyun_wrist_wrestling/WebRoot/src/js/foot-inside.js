...
PublickFn.prototype = {
    constructor: PublickFn, replace_url: function () {
    }, href_url: function () {
    }, ajaxFn: function () {
        var a = this;
        $.ajax({
            type: "get", url: this.basicUrl + this.url, data: this.data, dataType: dataType, beforeSend: function (b) {
            }, success: function (e) {
                if (e.result == "ok") {
                    var f = e.values || null;
                    return a.analysisAjax(f)
                } else {
                    var d = e.errormsg;
                    return noticeSetTimeoutPublick(d)
                }
            }, complete: function (b) {
            }, error: function () {
            }
        })
    }, analysisAjax: function (b) {
        return this.callback(b)
    }
};
//快速点击
function testInput(val, RE, _notive, num, $val) {
    var len = val.length;

    if (val.match(RE) && len >= 1 && len < num) {
        return val;
    } else {
        $val.val("");
